...
{
	ImGuiContext& g = *GImGui;
	return g.RenderDrawData.Valid ? &g.RenderDrawData : NULL;
}
...
{
    return ImGui::Begin(name, p_open, ImVec2(0.f, 0.f), .5f, flags);
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrentLineTextBaseOffset) // 尝试垂直对齐较小/没有填充的按钮，以便文本基线匹配（位hacky，因为它不应该是标志）
...
void ImGui::ImageS(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& X, const ImVec2& Y, const ImVec4& tint_col, const ImVec4& border_col)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	if (border_col.w > 0.0f)
	{
		window->DrawList->AddRect(X + bb.Min, X + bb.Max, GetColorU32(border_col), 0.0f);
		window->DrawList->AddImage(user_texture_id, X + bb.Min + ImVec2(1, 1), X + bb.Max - ImVec2(1, 1), uv0, uv1, GetColorU32(tint_col));
	}
	else
	{
		window->DrawList->AddImage(user_texture_id, bb.Min, bb.Max, uv0, uv1, GetColorU32(tint_col));
...
// frame_padding <0：使用样式中的FramePadding（默认）
// frame_padding = 0：没有框架
// frame_padding> 0：设置框架大小
//使用的颜色是按钮颜色。
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
	...
	//默认使用纹理ID作为ID。 用户仍然可以推送字符串/整数前缀。
		 //我们可以散列大小/ uv来创建一个唯一的ID，但这会阻止用户设置UV动画。
...
bool ImGui::CheckboxS(const char* label, bool* v)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
